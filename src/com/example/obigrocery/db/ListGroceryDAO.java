...
public class ListGroceryDAO {
	...
	// Database fields
	 ...
	 private String[] allColumns = { DBTools.COLUMN_LISTGROCERY_LISTGROCERYID,
			 ...
			 DBTools.COLUMN_LISTGROCERY_PRODUCTID, DBTools.COLUMN_LISTGROCERY_UNITS,
			 DBTools.COLUMN_LISTGROCERY_AMOUNT, DBTools.COLUMN_LISTGROCERY_ISPURCHASED};
	...
	public ListGrocery createListGrocery(long listId, long productId, float amount, 
		 ...
		 long insertId = database.insert(DBTools.TABLE_LISTGROCERY, null, values);
		 
		 Cursor cursor = database.query(DBTools.TABLE_LISTGROCERY, allColumns,
				 DBTools.COLUMN_LISTGROCERY_LISTGROCERYID + " = " + insertId, null, null,
				 null, null);
		 cursor.moveToFirst();
	...
	public void deleteListGeocerybyListId(long listId){
		 ...
		 if (listedProducts != null && !listedProducts.isEmpty()) {
			 for (ListGrocery l : listedProducts) {
				 ...
				 //database.delete(DBTools.TABLE_LISTGROCERY, DBTools.COLUMN_LISTGROCERY_PRODUCTID 
				 //		 + " = "+ productId , null);
			 }		 
		 }
	}
	...
	public List<ListGrocery> getListGroceryByCategory(String categoryName){
		 ...
		 if (products != null && !products.isEmpty()) {
			 for (Products p : products){
				 List<ListGrocery> listGrocery = getListGeocerybyProductId(p.getId());
	...
	public boolean updateListGrocery(long listGroceryId, long listId, long productId,
			  String units, float amount, int isPurchased){
		  ...
		  return database.update(DBTools.TABLE_LISTGROCERY, args, 
	   			DBTools.COLUMN_LISTGROCERY_LISTGROCERYID + "="+ listGroceryId, null)>0;
	  }
	...
	protected ListGrocery cursorToListGrocery(Cursor cursor) {		
		...
		listGrocery.setProductID(cursor.getLong(1));
